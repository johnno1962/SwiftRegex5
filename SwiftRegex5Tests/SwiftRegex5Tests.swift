...
class SwiftRegex5Tests: XCTestCase {
    ...
    func testExample() {
        ...
        if let (initial, remainder): (String, String) = str.firstMatch(of: "(\\w)(\\w*)") {
            XCTAssert((initial, remainder) == ("H", "ello"), "extract match")
        }
        ...
        numbers[#"(\d+) (\d+)-(\d+)"#, 1] = ["777"]
        XCTAssertEqual(numbers, "phone: 777 000-1234 fax: 444 000-1234")
        ...
        XCTAssertEqual(str1[#"(\w+) (\w+)"#], "hello single")
        XCTAssertEqual(str1[#"(?:\w+) (\w+)"#], "single")
        XCTAssertEqual(str1[#"(\w+) (\w+)"#, 1], "hello")
        XCTAssertEqual(str1[#"(\w+) (\w+)"#, 2], "single")
        ...
        case match["(\\w)(\\w*)"]:
            ...
            print("\(first)~\(rest)")
            XCTAssertEqual("\(first)~\(rest)", "S~ALUT")
        default:
            XCTFail()
            break
        }
